...
SPEC_FILE=$(rpm -E %_specdir)/userver-$VERSION-$RELEASE.spec
...
SOURCE_DIR="$RES_TMP/userver-$VERSION"
rm -rf "$SOURCE_DIR"
mkdir -p "SOURCE_DIR"
rsync -av --exclude '/build' --exclude ".git" --exclude ".svn" ./ "$SOURCE_DIR"
rm -f "$RPM_SOURCES_DIR/userver-$VERSION-$RELEASE.tar.gz"
pushd "$RES_TMP"
tar -czvf "$RPM_SOURCES_DIR/userver-$VERSION-$RELEASE.tar.gz" "userver-$VERSION"
popd

SOURCE="userver-$VERSION-$RELEASE.tar.gz"
#SOURCE="https://github.com/yoori/userver/archive/refs/tags/v$VERSION.tar.gz"
